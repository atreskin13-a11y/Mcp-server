...
def get_chroma_collection(collection_name: Optional[str] = None):
    try:
        ...
        raise RuntimeError("Библиотека chromadb не установлена. Установите: pip install chromadb")
    ...
    name = collection_name or settings["default_collection_name"]
    return client.get_or_create_collection(name=name)


def try_clear_collection(collection) -> None:
    try:
...
def build_rag_tool(
    ...
    """
    Создает RAG по указанной директории проекта с использованием ChromaDB.
    Читает файлы, разбивает их на чанки и сохраняет в коллекцию.
    """
    try:
        settings = load_rag_settings()
        ...
        collection = get_chroma_collection(final_collection_name)

        if clear_collection:
            try_clear_collection(collection)
        ...
        for file_path in files:
            try:
                ...
                for i, chunk in enumerate(chunks):
                    batch_ids.append(make_chunk_id(file_path, i))
...
def query_rag_tool(
    ...
    try:
        ...
        collection = get_chroma_collection(final_collection_name)
        result = collection.query(query_texts=[question], n_results=final_top_k)
...
def build_rag_from_collected_tool(
    ...
    Предполагается, что collected_file был получен через collect_project_tool.
    """
    try:
        settings = load_rag_settings()
